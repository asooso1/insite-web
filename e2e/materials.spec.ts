...
test.describe("자재 목록 페이지", () => {
  ...
  test("페이지 제목이 표시되어야 함", async ({ page }) => {
    // PageHeader title 확인
    await expect(page.getByRole("heading", { name: "자재 목록" })).toBeVisible();
  });

  test("페이지 설명이 표시되어야 함", async ({ page }) => {
    // PageHeader description 확인
    await expect(page.getByText("자재 재고를 관리합니다.")).toBeVisible();
  });

  test("신규 등록 버튼이 표시되어야 함", async ({ page }) => {
    // 새 자재 버튼 확인
    const addButton = page.getByRole("button", { name: /새 자재/ });
  ...
  test("검색 필드가 표시되어야 함", async ({ page }) => {
    // 검색 입력 필드 확인
    await expect(page.getByPlaceholder("자재명 검색...")).toBeVisible();
  });

  test("상태 필터 탭이 표시되어야 함", async ({ page }) => {
    // 상태 탭 확인 (전체, 운영, 준비, 폐기)
    await expect(page.getByRole("tab", { name: "전체" })).toBeVisible();
    await expect(page.getByRole("tab", { name: "운영" })).toBeVisible();
    await expect(page.getByRole("tab", { name: "준비" })).toBeVisible();
  ...
  test("테이블 헤더가 올바른 열을 포함해야 함", async ({ page }) => {
    ...
    await expect(page.getByRole("columnheader", { name: "자재명" })).toBeVisible();
  ...
  test("검색어 입력 시 테이블이 업데이트되어야 함", async ({ page }) => {
    ...
    // 검색어 입력
    await page.getByPlaceholder("자재명 검색...").fill("자재");
  ...
  test("자재명 클릭 시 상세 페이지로 이동", async ({ page }) => {
    // 데이터 로드 대기
    await page.waitForLoadState("networkidle");

    // 첫 번째 자재명 링크 찾기
    const firstMaterialLink = page.locator("tbody tr").first().locator("button.text-primary").first();

    // 링크가 존재하는지 확인
    if (await firstMaterialLink.isVisible()) {
      // 링크 클릭
      await firstMaterialLink.click();

      // URL이 /materials/{id} 형태로 변경되는지 확인
  ...
  test("전체 탭에서 모든 상태 자재가 표시되어야 함", async ({ page }) => {
    // 데이터 로드 대기
    await page.waitForLoadState("networkidle");
