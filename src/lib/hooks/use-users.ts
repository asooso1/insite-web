...
export const userKeys = {
  ...
  registerRoles: () => [...userKeys.all, "registerRoles"] as const,
};
...
export function useEditUser() {
  ...
  return useMutation({
    mutationFn: (data: UserVO) => editUser(data),
...
export function useResetPassword() {
  return useMutation({
    mutationFn: (data: UserVO) => resetPassword(data),
  });
}
 ...
 * 사용자 엑셀 다운로드 훅
 */
export function useDownloadUserExcel() {
  return useMutation({
    mutationFn: async (params: SearchUserVO) => {
      const blob = await downloadUserListExcel(params);
      ...
      triggerExcelDownload(blob, `users-${getTodayDateString()}.xlsx`);
    },
  });
}
