...
#### ⚠️ 건물 폼 미구현 항목 (추후 개선 필요)
| 항목 | 현황 | 필요 작업 | 우선순위 |
|------|------|-----------|----------|
| 고객사 선택 (`companyId`) | 숫자 직접 입력 | Combobox로 고객사 검색/선택 (useAdminBuildings 또는 클라이언트 목록 API 활용) | 🔴 High |
| 건물 용도 선택 (`useType2Id`) | 숫자 직접 입력 | 2단계 계층 Select (대분류→소분류) — csp-was 용도분류 API 훅 신규 구현 필요 | 🔴 High |
| 광역/거점 선택 (`wideAreaId`, `baseAreaId`) | 숫자 직접 입력 | CascadingSelect — 광역 선택 후 해당 거점 로드, csp-was 지역 API 훅 신규 구현 필요 | 🔴 High |
| 주소 검색 | 직접 텍스트 입력 | 카카오/다음 우편번호 API 연동으로 자동 완성 | 🟡 Medium |
| 지도 위치 선택 | 위도/경도 텍스트 입력 | 카카오맵 API 연동으로 클릭 선택 | 🟡 Medium |
